...
- Verify previous slices in **actual code** under `backend/<project>/`.
- Read `docs/conventions/modular-monolith-fastify-react.md`.

## Research (mandatory)

1. Read slice + slices README + technical design.
2. Explore `backend/<project>/` — exact files, patterns, tests.
...
- **Relevant files** — modify / new / reference (paths under `backend/<project>/`)
...
- **Validation commands** — for `his-global-south` always include CI twins: `npm run lint`, `npx tsc -b` (frontend), `npm run build` in `backend/`, plus vitest paths. Do not list only `vite` / unit tests.
