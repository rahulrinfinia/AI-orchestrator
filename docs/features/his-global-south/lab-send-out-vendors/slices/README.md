...
| **G3 status** | **Pending** — fill Approval on each slice before implement |
...
| Slice | Spec | Goal |
|-------|------|------|
| LSO-1 | [slice-1.md](./slice-1.md) | Lab vendor CRUD API + org-setup UI (`super_admin` writes) |
| LSO-2 | [slice-2.md](./slice-2.md) | Assign vendor on send-out orders + tracker badges/status |
...
Fill the **Approval** block on **LSO-1** first, then say: **plan slice 1** or **implement slice 1**. Do not implement until Approval is filled.
